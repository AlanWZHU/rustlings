...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        // team_1.
        // zhujunhao 为什么 不可变的 team_1_name move 后不可在用 因为单一所有权机制 但是 如何不再clone呢？
        
        scores.entry(team_1_name.clone()).
        and_modify(|t|{t.goals_conceded+=team_2_score;t.goals_scored+=team_1_score}).
        or_insert(Team { name: team_1_name, goals_scored: team_1_score, goals_conceded: team_2_score });
        scores.entry(team_2_name.clone()).
        and_modify(|t|{t.goals_conceded+=team_1_score;t.goals_scored+=team_2_score}).
